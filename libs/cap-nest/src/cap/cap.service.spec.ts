...
describe('CapService (unit)', () => {
    ...
    beforeEach(() => {
        ...
        subscriber = {
            consume: jest.fn().mockImplementation(async (topic: string, group: string, onMessage: any) => {
    ...
    it('subscribe - handler processed successfully and persisted', async () => {
        ...
        await (subscriber as any).__trigger('topic-x', 'group-1', { foo: 'bar' });
    ...
    it('subscribe - handler failure schedules retry', async () => {
        ...
        await (subscriber as any).__trigger('topic-retry', 'group-r', { z: 9 });
