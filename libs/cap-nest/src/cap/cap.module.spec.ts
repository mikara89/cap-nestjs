...
import { PUBLISH_STORAGE, RECEIVED_STORAGE } from './abstractions/storage.interface';
...
describe('CapModule builders', () => {
    it('forInMemory returns DynamicModule with adapters containing provider tokens', () => {
        ...
        const hasPublishStorage = providers.some((p: any) => p && (p.provide === PUBLISH_STORAGE || p.useClass));
        const hasReceivedStorage = providers.some((p: any) => p && (p.provide === RECEIVED_STORAGE || p.useClass));
        const hasPublisher = providers.some((p: any) => p && p.provide === PUBLISHER);
        const hasSubscriber = providers.some((p: any) => p && p.provide === SUBSCRIBER);
