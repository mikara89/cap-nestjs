...
export class CapDashboardModule {
    static forRoot(opts: CapDashboardModuleOptions): DynamicModule {
        ...
        if (opts.serveStatic !== false) {
            ...
            if (found) {
                try {
                    // require @nestjs/serve-static at runtime so tests without that package don't fail
                    // eslint-disable-next-line @typescript-eslint/no-var-requires
                    const { ServeStaticModule: SSM } = require('@nestjs/serve-static');
                    const resolved = require('path').resolve(found as string);
                    imports.push(
                        SSM.forRoot({
                            rootPath: resolved,
        ...
        if (routeBase && routeBase !== '') {
            ...
            try {
                // eslint-disable-next-line @typescript-eslint/no-var-requires
                const { Controller: ControllerDecorator } = require('@nestjs/common');
                ...
                ControllerDecorator(routeBase)(controllerToRegister);
            } catch (err) {
